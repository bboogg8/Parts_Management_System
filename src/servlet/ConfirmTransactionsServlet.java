...
public class ConfirmTransactionsServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (flag == 0){//检查供应商状态
                pStatement = connection.prepareStatement("select supplierStatus,buyId,partNum,supplyId from management.transaction WHERE id=?");
            }else{
                pStatement = connection.prepareStatement("select customerStatus,buyId,partNum,supplyId from management.transaction WHERE id=?");
            }
            ...
            if (rowSet.getInt(1) == 1){//代表双方都已经同意了
                ...
                pStatement = connection.prepareStatement("update management.supply set partNum = partNum - ? where id=?");
