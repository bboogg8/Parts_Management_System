...
public class ShowDemandServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            if (customerId == -1){//id为 -1 代表查询所有用户的需求
                pStatement = connection.prepareStatement("select partId,partPrice,partNum,created,customId,id,status from management.buy");
            }else{//否则查询某一具体用户的需求
                pStatement = connection.prepareStatement("select partId,partPrice,partNum,created,customId,id,status from management.buy where customId=?");
            ...
            while (rowSet.next()){
                ...
                pStatement = connection.prepareStatement("select name from management.parts where id = ?");
                ...
                pStatement = connection.prepareStatement("select name from management.customer where id = ?");
