...
public class ShowTransactionServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            pStatement = connection.prepareStatement("select suggestion,management.transaction.id,customerStatus,supplierStatus,buyId,management.customer.id,management.customer.name,management.supplier.id,management.supplier.name, management.parts.id,management.transaction.partNum,management.transaction.partPrice,parts.name from management.transaction,management.buy,management.supply,management.customer,management.supplier,management.parts where supplyId = supply.id and buyId = buy.id AND customer.id = buy.customId and supplier.id = supply.supplierId and parts.id = supply.partId" + userLimit + supplierLimit);
